...
### Course Outcome and Experience
I have met course outcome three, as I was able to use algorithmic principles to solve the problem of efficiency in looking up inventory. Not only is looking up an item more efficient, but adding, updating, and deleting items also become more efficient. The application can also quickly decern if an item exists in the database, and add or update the item accordingly without having to iterate through the inventory.
As I was creating this enhancement, I had issues changing a lot of my code to work with the new HashMap, as their was a lot to change and it was easy to forget what still needed to be reworked. I also had to change my repository to a singleton, as after changing to use a HashMap, I found out on creating the repository each time, it created two separate HashMaps in my InventoryActivity and my ItemDetailsActivity since each activity created its own repository. [Here](CS-499Milestone3.pdf) is my narrative for enhancement two. 
...
### Course Outcome and Experience
This component showcases my skills in backend programming and database design, as I had to correctly use HTTP requests and responses to communicate with the backend for CRUD operations for inventory in a database. The course outcome I planned to meet was outcome three and one, and I believe I had met it. Switching from a local database to a centralized one solves an industry-specific goal, while managing trade-offs of a more complex system, as well as enabling collaboration between workers through the use of a cloud-based database. In creating this enhancement, I ran into an issue that helped me learn more about permissions needed for android applications. Firstly, my recycler stopped showing the items after updating to an API with node.js, although it looked like it should work since the phone itself could connect to the local host and showed the added JSON data. What I found out was that android was blocking my HTTP request as I did not realize it needed permission in the android manifest xml. [Here](MilestoneFour.pdf) is my narrative for enhancement three. 
